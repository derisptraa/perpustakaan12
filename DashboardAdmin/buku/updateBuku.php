...
$reviewData = queryReadData("SELECT buku.*, kategori_buku.nama_kategori AS kategori 
                            FROM buku 
                            JOIN kategori_buku ON buku.id_kategori = kategori_buku.id_kategori 
                            WHERE buku.id_buku = '$review'")[0];
      ...
      <form action="" method="post" enctype="multipart/form-data" class="mt-3 p-2">
        ...
        <div class="input-group mb-3">
          ...
          <select class="form-select" id="inputGroupSelect01" name="kategori">
            <option selected value="<?= $reviewData["id_kategori"]; ?>">
              <?= $reviewData["kategori"]; ?>
            </option>
            <!-- Loop untuk kategori lain -->
            <?php foreach ($kategori as $item) : ?>
              <?php if ($item["id_kategori"] != $reviewData["id_kategori"]) : ?>
